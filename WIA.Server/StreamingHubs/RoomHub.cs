    ...
    public class RoomHub(RoomContextRepository roomContextRepository) : StreamingHubBase<IRoomHub, IRoomHubReceiver>, IRoomHub
    {
        ...
        public async Task EnemyHealthAsync(string enemID, float giverATK, List<EnumManager.DEBUFF_TYPE> debuffType)
        {
            lock (roomContextRepository) // 排他制御
            {
                ...
                //        // 参加者に受け取ったIDの敵が受け取ったHPになったことを通知
            ...
            /// <returns></returns>
            bool LotteryIllegalScript(float value)
            {
                Random rand = new Random();
            ...
            /// <returns></returns>
            async Task DeleteEnemyAsync(string enemId)
            {
            ...
            void DeleteEnemyData(string uniqueId)
            {
                lock (roomContextRepository)
                {
            ...
            /// <returns></returns>
            async Task ApplyDamageToEnemyAsync(string enemID, int dmgAmount)
            {
                lock (roomContextRepository) // 排他制御
                {
                    // 敵被弾データを新しく作成
        ...
        //{
            ...
            //                if (status.attackSpeedFactor >= MAX_REGENERATE) status.attackSpeedFactor = MAX_REGENERATE;
            //                break;
                //    }
                //}
