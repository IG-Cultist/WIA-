...
public class PieChart : MonoBehaviour
{
    ...
    [SerializeField]  GameObject succsessColor;
    [SerializeField]  GameObject failColor;

    private void Start()
    {
        succsessColor.GetComponent<Image>().color = _color[0];
        failColor.GetComponent<Image>().color = _color[1];
    }
    ...
    void Update()
    {
        if (updateFlg)
        {
            for (int i = SkipCnt; i >= 0; i--)
            {
                ...
                {
                    ChildObject[i].GetComponent<Image>().enabled = true;
