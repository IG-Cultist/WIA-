...
public class FlowerPotManager : MonoBehaviour
{
    ...
    {
        ////LookDownCameraを有効化する
        //cameraManager.TurnOnLookDownCam();

        while (true)
        {
            // 生成位置を決める
            generateNumber = Random.Range(0, randomSpawnPoint.Count); // 生成位置をspawnPointPosの中から決める
        ...
        nowSpawnList.Add(generateNumber);

        if (RoomModel.Instance)
        {
            gameManager.SpawnObj(randomSpawnPoint[generateNumber].transform.position);
        }
        else
        {
